...
// Estados para almacenar la latitud y longitud
    ...
    val fusedLocationClient = remember {
        LocationServices.getFusedLocationProviderClient(context) }  // Launchers para pedir permisos
    val requestPermissionLauncher = rememberLauncherForActivityResult(
        ActivityResultContracts.RequestPermission()
    ) { isGranted ->
        if (isGranted) { // Si el permiso está otorgado, obtener la ubicación
    ...
    Column(
            ...
            onClick = {
                when (PackageManager.PERMISSION_GRANTED) {
                    ContextCompat.checkSelfPermission(context,
                        Manifest.permission.ACCESS_FINE_LOCATION) -> {  // Si el permiso ya está otorgado, obtener la ubicación
                        getLocation(fusedLocationClient) { lat, lon ->
                            latitude = lat
                            longitude = lon
                        }
                    }
                    else -> {  // Si el permiso no está otorgado, pedirlo
        ...
        Text(text = "Tu latitud es: $latitude")
        Text(text = "Tu longitud es: $longitude")
    }
}
